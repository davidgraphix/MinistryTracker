...
import { FaUserPlus, FaCheckCircle, FaTimesCircle, FaChartPie, FaUsers, FaUser } from "react-icons/fa";
...
const Dashboard = () => {
  ...
  const newWorkersLast7Days = useMemo(() => {
    ...
    return workers.filter((worker) => dayjs(worker.createdAt).isAfter(sevenDaysAgo)).length;
  }, [workers]);
  ...
  const { presentCount, totalCount } = workers.reduce(
    (acc, worker) => {
      acc.presentCount += worker.attendanceRecords.filter((rec) => rec.status === "Present").length;
      acc.totalCount += worker.attendanceRecords.length;
      return acc;
    },
    { presentCount: 0, totalCount: 0 }
  );
  const overallAttendance = totalCount > 0 ? Math.round((presentCount / totalCount) * 100) : 0;

  const sortedByAttendance = [...workers].sort((a, b) => {
    const aPresents = a.attendanceRecords.filter((rec) => rec.status === "Present").length;
    ...
    <div className="flex h-screen bg-gray-100">
      <main className="flex-1 p-5 overflow-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
          <motion.div
            className="bg-white rounded shadow p-4 flex items-center transition-all duration-300 hover:scale-105"
          >
        ...
        <div className="bg-white rounded shadow p-4">
          <h2 className="text-xl font-bold mb-4">Attendance Leadership Board</h2>
